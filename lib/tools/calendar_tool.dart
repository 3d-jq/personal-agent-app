...
class CalendarTool extends AgentTool {
  ...
  Map<String, dynamic> get parameters => {
    'type': 'object',
    'properties': {
      'action': {'type': 'string', 'description': '操作: query/add/delete', 'enum': ['query', 'add', 'delete']},
      ...
      'date': {'type': 'string', 'description': '日期 YYYY-MM-DD，如 2026-06-15。优先使用此参数'},
  ...
  Future<bool> _ensurePerm() async {
    ...
    if (s.isDenied || s.isPermanentlyDenied) s = await Permission.calendarFullAccess.request();
    return s.isGranted;
  }
  ...
  Future<String> execute(Map<String, dynamic> args) async {
    ...
    try {
      switch (action) {
        case 'query':
          final s = (args['start_ms'] as num?)?.toInt() ?? DateTime.now().subtract(const Duration(days: 1)).millisecondsSinceEpoch.toInt();
          final e = (args['end_ms'] as num?)?.toInt() ?? DateTime.now().add(const Duration(days: 7)).millisecondsSinceEpoch.toInt();
          return await _channel.invokeMethod<String>('query', {'startMs': s, 'endMs': e}) ?? '查询失败';
        case 'add':
          ...
          if (ds != null && ds.isNotEmpty) {
            ...
            } catch (_) { return '日期格式错误，请用 YYYY-MM-DD，如 2026-06-15'; }
          } else {
            s = (args['start_ms'] as num?)?.toInt() ?? DateTime.now().millisecondsSinceEpoch.toInt();
            e = (args['end_ms'] as num?)?.toInt() ?? s + 3600000;
          }
          return await _channel.invokeMethod<String>('add', {'title': t, 'description': args['description'] ?? '', 'startMs': s, 'endMs': e}) ?? '添加失败';
        case 'delete':
