...
class NoteExportService {
  ...
  static Future<void> exportToWord(Note note) async {
    ...
    final fileName = '笔记_${note.title}_${DateTime.now().millisecondsSinceEpoch}.html';
  ...
  static String _buildHtml(Note note) {
    ...
    buffer.writeln('body { font-family: "Microsoft YaHei", "微软雅黑", sans-serif; max-width: 800px; margin: 0 auto; padding: 40px; color: #333; line-height: 1.8; }');
    buffer.writeln('h1 { font-size: 24px; border-bottom: 2px solid #0F7B6C; padding-bottom: 10px; color: #37352F; }');
    ...
    buffer.writeln('code { background: #f5f5f5; padding: 2px 6px; border-radius: 4px; font-family: monospace; }');
    buffer.writeln('pre { background: #1e1e1e; color: #d4d4d4; padding: 16px; border-radius: 8px; overflow-x: auto; }');
    buffer.writeln('pre code { background: none; padding: 0; color: inherit; }');
    buffer.writeln('blockquote { border-left: 4px solid #0F7B6C; padding-left: 16px; color: #666; margin: 16px 0; }');
    buffer.writeln('ul, ol { padding-left: 24px; }');
    buffer.writeln('li { margin: 4px 0; }');
    buffer.writeln('.date { color: #999; font-size: 12px; margin-bottom: 20px; }');
    buffer.writeln('</style>');
    buffer.writeln('</head>');
    buffer.writeln('<body>');
  ...
  static String _convertLine(String line) {
    ...
    if (trimmed.startsWith('### ')) return '<h3>${_escapeHtml(trimmed.substring(4))}</h3>';
    if (trimmed.startsWith('## ')) return '<h2>${_escapeHtml(trimmed.substring(3))}</h2>';
    if (trimmed.startsWith('# ')) return '<h1>${_escapeHtml(trimmed.substring(2))}</h1>';
    ...
    if (trimmed.startsWith('```')) return trimmed.contains('```') ? '<pre><code>' : '</code></pre>';

    final imgMatch = RegExp(r'!\[.*?\]\((file://[^\s)]+)\)').firstMatch(trimmed);
    if (imgMatch != null) {
    ...
    text = text.replaceAllMapped(RegExp(r'\*\*(.+?)\*\*'), (m) => '<strong>${m.group(1)}</strong>');
    text = text.replaceAllMapped(RegExp(r'\*(.+?)\*'), (m) => '<em>${m.group(1)}</em>');
    text = text.replaceAllMapped(RegExp(r'`(.+?)`'), (m) => '<code>${m.group(1)}</code>');
