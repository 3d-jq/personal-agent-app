...
class DailyCardProvider extends ChangeNotifier {
  ...
  Future<void> generate() async {
    ...
    try { final w = WeatherTool()..apiKey = dotenv.env['AGNES_API_KEY'] ?? ''; buf.writeln('天气: ${await w.execute({'city': '北京', 'units': 'metric'})}'); } catch (_) {}
    ...
    if (!ctx.contains('天气') && !ctx.contains('日程') && !ctx.contains('记忆')) {
      ...
      try {
        final ai = AIService(baseUrl: 'https://apihub.agnes-ai.com/v1', apiKey: dotenv.env['AGNES_API_KEY'] ?? '', providerName: '', model: 'agnes-2.0-flash');
