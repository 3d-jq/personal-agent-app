...
- **修复流式输出时主界面卡顿（点侧边栏 / 进入子页面掉帧）**：根因有二 —— ① 流式期间 `_AIBubble` 每个 token 都**全量解析整条 markdown**（原有 32ms 节流仍每帧重解析长文本），占满主 isolate，Drawer 打开 / 进入子页面时后台解析与界面渲染抢主线程；② 流式自动贴底每帧 `jumpTo` 与 Drawer 打开动画抢主 isolate。修法：① 流式期间（`msg.isStreaming`）改用**纯文本 `Text`** 渲染（零解析开销，文字平滑出现），流结束（`isStreaming=false`）再切回富文本；② Drawer 打开时（`onDrawerChanged` 置 `_drawerOpen`）暂停自动贴底滚动，消除每帧 `jumpTo` 抢占。新增回归测试锁定「流式纯文本 / 结束富文本」分流，防被改回全量解析。
