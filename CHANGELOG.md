...
### 🐛 侧边栏修复
- 关闭手势对称：快速左甩（velocity < -500）可关闭侧边栏，不再必须拖过半屏
- 垂直滚动误触：手指水平漂移 < 垂直 80% 时忽略，列表滚动不再意外拉出侧边栏
- 返回按钮图标：`Icons.close`（叉号）→ `Icons.arrow_back`（返回箭头），语义对齐

### ✨ 发送后用户消息顶到视口顶部
- 发送消息后自动将刚发的用户消息滚动到屏幕最上方，AI 流式回复在下方展开
- 解决 `AnimatedSwitcher` key 复用 + 真机 `loadAll` 跨帧导致的滚动失效

### 🎨 Token 编辑单价按钮等宽化
- 计费方式切换按钮不再按文字长度分配宽度，"按 token / 按次"始终等宽

### 📋 代码审查标准
- 新增 `CODE_REVIEW.md`：通用清单 / 专项清单 / 性能红线 / 测试规范 / 审查礼仪

### 🧪 测试
- 新增 2 条测试（`chat_send_scroll_test.dart`），全量 496 通过
...
- 点对话立即切「加载对话中」骨架屏；真正耗时的切会话（DB 读 + 气泡重建）推迟到抽屉关闭动画结束后再执行。
- 双触发 + 兜底定时器：`onDrawerChanged(false)` 回调与 `Future.delayed(AppDurations.expressive)` 兜底，`_pendingSwitchId` 空值去重，绝不双触发。

### 🔗 UI 视口窗口与模型上下文解耦
- 模型现在看到**全部**历史上下文，按 80% 阈值触发压缩（此前误把 UI 窗口条数当模型数据源，压缩形同虚设）。
- `MessageWindow.loadFullHistory()` 取 DB 全量；`ChatController.buildSendView()` 以全量 + 合并未落盘消息构造发送视图。
- `windowSize` 明确只管 UI 首屏显示/加载性能，**不参与模型上下文**。

### ⚡ UI 视口窗口 40→30
- 纯 UI 加载性能调优（首屏/翻页各少加载 10 条），不影响模型上下文。

## v1.4.33 — 聊天性能专项优化 + 版本更新 (2026-07-12)

### ⚡ 流畅度优化
- 快速滚动降级：检测滚动速度 > 阈值 → 纯文本占位，停止 250ms 恢复
- Markdown 懒解析：非流式消息首帧纯文本 → 下一帧完整渲染
- 渐进式消息恢复：先出 30 条秒显 → 后台补齐剩余
- 会话分页 API：loadAll/loadChatSessions 支持 offset/limit

### ✨ 新功能
- 「n 条新消息」浮条：看历史时来新消息，底部显示计数胶囊
- 版本更新检查：设置页 → 远端 latest.json → 下载进度 → 系统安装

## v1.4.32 — 提示词全量逻辑引导 + 工具描述场景化 + Bug 修复 (2026-07-11)

### 🧠 提示词体系全面改写
- `<rules>` / `<persona_constraints>` / `<skill_usage>` 从编号操作指令改为 Markdown 分节自然引导（"你不知道的事情很多，世界每天都在变……"）
- `<first_meeting>` 改为场景化开场引导（"你不是在填表，你是在认识一个人"）
- 41 个工具 `.txt` 描述从功能说明改为场景引导（"什么时候用、为什么用、用完怎么办"）
- `<role>` 动态读取 USER.md「怎么叫我」字段，不再写死
- 提示词层面彻底去掉所有硬编码 DWeis（UI 产品名不动）
- 修复编号重复 bug（安全规则 11,12 覆盖记忆规则 11,12）

### 🐛 Bug 修复
- PressableScale 侧边栏点击区域只响应 > 图标（GestureDetector 默认 deferToChild 被 Material 截断）
- 工具失败后剩余工具过早显示 > 而不是扫光动画（`_buildProcessLine` 抢跑）
- `deep_search` 补上中文 toolLabel「深度搜索」
- 撤回未验证的浏览器工具模块

### 👤 首次见面 AI 命名
- USER.md 新增「怎么叫我」字段
- SOUL.md 默认模板名称从 DWeis → 你的 AI 助手
- 首次见面引导用户给 AI 起名字
